...
def main() -> None:  # pragma: no cover — сетевой запуск, проверяется живьём
    ...
    app = (
        ...
        # concurrent_updates: PTB обрабатывает апдейты параллельно
        # (без этого _run_generation на 4 минуты блокирует ВСЕ тапы юзера
        # → его callback_query истекают по таймауту → «Query is too old»).
